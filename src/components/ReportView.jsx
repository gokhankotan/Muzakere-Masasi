...
export default function ReportView({ onBack, sessionCode, lang = "tr" }) {
  ...
  if (!reportData) {
    return (
      <div style={{ maxWidth: "700px", margin: "4rem auto", padding: "2rem", background: "#fff", borderRadius: "12px" }}>
        <h2 style={{ color: "#111" }}>{lang === "tr" ? "Hata" : "Error"}</h2>
        <p style={{ color: "#555" }}>{lang === "tr" ? "Rapor verileri alınamadı." : "Failed to retrieve report data."}</p>
  ...
  return (
    <div style={{ background: "#e8e8e8", minHeight: "100vh", padding: "2rem 1rem" }}>

      {/* Yazdirma Disi Kontrol Cubugu */}
      <div className="no-print" style={{ maxWidth: "900px", margin: "0 auto 1.5rem auto", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(15,8,30,0.85)", border: "1px solid rgba(168,85,247,0.25)", borderRadius: "12px", padding: "0.75rem 1.25rem" }}>
        <button onClick={onBack} className="btn" style={{ background: "transparent", borderColor: "rgba(255,255,255,0.2)", color: "#ccc", display: "flex", alignItems: "center", gap: "0.4rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.6rem" }}>
          ...
          <button onClick={handlePrint} className="btn" style={{ background: "#7c3aed", display: "flex", alignItems: "center", gap: "0.4rem" }}>
      ...
      <div className="report-container">
          ...
          <table className="report-table" style={{ marginTop: "1rem" }}>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={{ fontSize: "0.85rem", color: "#555" }}>{analysis?.voteCompletionRate < 20 ? (lang === "tr" ? "⚠ Düsük tamamlama — güvenilirlik etkisi" : "⚠ Low completion — reliability impact") : (lang === "tr" ? "Yeterli oy verisi" : "Sufficient vote data")}</td>
              </tr>
              <tr>
                <td>{lang === "tr" ? "Belirsiz Katilimci" : "Ambiguous Participants"}</td>
                <td><strong>{ambiguousCount}</strong></td>
                <td style={{ fontSize: "0.85rem", color: "#555" }}>{lang === "tr" ? "Herhangi bir gruba dahil edilemeyen" : "Not clearly assigned to any group"}</td>
              </tr>
              {showVarianceWarning && (
                <tr style={{ background: "#fffbeb" }}>
                  <td colSpan={3} style={{ color: "#92400e", fontSize: "0.82rem" }}>
        ...
        {analysis?.insufficientData ? (
          <section className="report-section">
            <div style={{ textAlign: "center", padding: "3rem 1rem", border: "1px dashed #c4b5fd", background: "#faf5ff", borderRadius: "8px", color: "#7c3aed" }}>
              <div style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>📊</div>
              <h4 style={{ fontSize: "1.1rem", fontWeight: "bold", marginBottom: "0.5rem" }}>{lang === "tr" ? "Analiz Icin Yetersiz Veri" : "Insufficient Data for Analysis"}</h4>
              <p style={{ fontSize: "0.88rem", color: "#6b7280", maxWidth: "400px", margin: "0 auto", lineHeight: 1.6 }}>
          ...
          <>
            ...
            <section className="report-section no-print">
              ...
              {analysis?.polarizationHistory && analysis.polarizationHistory.length >= 2 ? (() => {
                ...
                return (
                  <div style={{ position: "relative", background: "#fff", border: "1px solid #e5e7eb", padding: "1rem", borderRadius: "8px" }}>
                    {hoveredHistoryPoint && (
                      <div style={{ position: "absolute", background: "#1e1b4b", color: "#fff", border: "1px solid #7c3aed", padding: "0.4rem 0.75rem", borderRadius: "6px", fontSize: "0.78rem", pointerEvents: "none", left: `${Math.min(hoveredHistoryPoint.x - 50, W - 110)}px`, top: `${hoveredHistoryPoint.y - 45}px`, zIndex: 20 }}>
                        <div style={{ fontWeight: "bold", color: "#a78bfa" }}>%{hoveredHistoryPoint.val} {lang === "tr" ? "Kutuplasma" : "Polarization"}</div>
                        <div style={{ fontSize: "0.68rem", color: "#d1d5db", marginTop: "0.1rem" }}>{new Date(hoveredHistoryPoint.time).toLocaleTimeString(lang === "tr" ? "tr-TR" : "en-US")}</div>
                      </div>
                    )}
                    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
                      {[0, 25, 50, 75, 100].map(level => { const y = (pT + cH) - (level / 100) * cH; return (<g key={level}><line x1={pL} y1={y} x2={W - pR} y2={y} stroke="#f3f4f6" strokeWidth={1} /><text x={pL - 8} y={y + 4} textAnchor="end" fontSize="9" fill="#9ca3af">%{level}</text></g>); })}
                      <path d={linePath} fill="none" stroke="#7c3aed" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
                      {pts.map((pt, i) => (<circle key={i} cx={pt.x} cy={pt.y} r={hoveredHistoryPoint && hoveredHistoryPoint.time === pt.time ? 6 : 4} fill={hoveredHistoryPoint && hoveredHistoryPoint.time === pt.time ? "#a78bfa" : "#7c3aed"} stroke="#fff" strokeWidth={1.5} style={{ cursor: "pointer" }} onMouseEnter={() => setHoveredHistoryPoint(pt)} onMouseLeave={() => setHoveredHistoryPoint(null)} />))}
                    </svg>
                  </div>
                );
              })() : (
                <div style={{ padding: "2rem 1rem", border: "1px dashed #d1d5db", textAlign: "center", color: "#9ca3af", borderRadius: "8px", fontStyle: "italic" }}>
            ...
            <tbody>
              {participants && participants.map((p, idx) => (
                <tr key={idx}>
                  ...
                  <td style={{ fontSize: "0.88rem", fontStyle: "italic", color: "#374151" }}>"{p.justification || (lang === "tr" ? "Belirtilmemis." : "Not specified.")}"</td>
                  <td style={{ textAlign: "center" }}>
                    <span style={{ background: p.isBot ? "#dbeafe" : "#ede9fe", color: p.isBot ? "#1e40af" : "#5b21b6", padding: "0.15rem 0.5rem", borderRadius: "4px", fontSize: "0.78rem", fontWeight: 600 }}>
        ...
        <div className="report-footer">
          ...
          <div style={{ marginTop: "0.5rem", fontSize: "0.75rem", color: "#d1d5db" }}>
